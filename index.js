				...
				.then(resp => {
					...
					if (!Array.isArray(resp) || !resp.length) {
						...
						if (URI.parse(url).host == config.url) {
							// yes it is
							logger.log(
								`invalid shorten request for ${url} from ${req.ip} - cannot shorten self`
							);
